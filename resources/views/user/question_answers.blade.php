                                        ...
                                        <a class="d-inline-block mr-2 mb-3 mb-md-0" href="#">
                                            @if ($answer->user_photo)
                                                <img src="{{ asset('images/user_photos/' . $answer->user_photo) }}"
                                                    class="mr-3 avater">
                                            @else
                                                <img src="{{ asset('images/user_photos/avatar.png') }}" class="mr-3 avater">
                                            @endif
                                        </a>
                                        <div class="media-body">
                                            <div class="d-flex justify-content-between align-items-center mb-4">
                                                <span>
                                                    ...
                                                    <small
                                                        class="text-black-800 ml-2 font-weight-600">{{ date('d M Y', strtotime($question->created_at)) }}</small>
                                                </span>
                                                @if ($answer->user_id == auth()->user()->id)
                                                    <form action="{{ route('question.answer.delete', $answer->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button class="delete text-danger border-0 bg-white"> <i
